...
client.once("ready", async () => {
  ...
  const forum = guild.channels.cache.find(c => c.name === "computer-vision" && c.type === 15);
  ...
  const content = readFileSync("content/computer-vision-welcome.md", "utf8");
  const chunks = splitContent(content);

  console.log(`Posting ${chunks.length} messages...`);

  const thread = await forum.threads.create({
    name: "👁️ START HERE: Computer Vision Guide & Resources",
